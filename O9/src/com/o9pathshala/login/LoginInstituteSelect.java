...
public class LoginInstituteSelect {
	...
	public void createProfile() throws JSONException {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				if(jsonObject.getString("id").startsWith("3"))
					sessionDTO.setType("Student");
				else if (jsonObject.getString("id").startsWith("2"))
					sessionDTO.setType("Faculty");
				else 
					sessionDTO.setType("Institute");
				...
				if("Student".equals(sessionDTO.getType())){
			...
			if(sessionDTOs.size() == 0){
				...
				alertDialogDTO.setMessage("Application is designed only for Students.\nKindly visit the website.");
