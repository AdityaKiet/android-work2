...
public class LoadMoreMyQuestionsAsynTask extends AsyncTask<String, String, Void> {
	private String sql;
	private String ip;
	...
	public LoadMoreMyQuestionsAsynTask(String sql, final Context context, RelativeLayout relativeLayout) {
		this.sql = sql;
		this.context = context;
		this.relativeLayout = relativeLayout;
		ResourceBundle rb = ResourceBundle.getBundle("network");
		ip = rb.getString("ip");
		this.sql = this.sql.replaceAll("LOWER_LIMIT", GlobalData.myQuestions.size()+ "");
		this.sql = this.sql.replaceAll("UPPER_LIMIT", (GlobalData.myQuestions.size() + 5) + "");
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				((Activity)context).runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						if(null != context && LoadMoreMyQuestionsAsynTask.this.isCancelled())
	...
	protected void onPreExecute() {
		...
		progressDialog.setOnCancelListener(new OnCancelListener() {
			...
			public void onCancel(DialogInterface arg0) {
				LoadMoreMyQuestionsAsynTask.this.cancel(true);
			}
		});
		super.onPreExecute();
	}
	...
	protected void onPostExecute(Void v) {
		...
		try{
			if(result.equals("false"))
				...
				GlobalData.myQuestions.addAll(questions);
				ListView listView = (ListView) relativeLayout.findViewById(R.id.listAllQuestions);
				AllQuestionsListAdapter adapter = new AllQuestionsListAdapter(context, GlobalData.myQuestions);
