...
public class GetMyQuestionAsynTask extends AsyncTask<String, String, Void>{
	private String sql;
	private String ip;
	...
	public GetMyQuestionAsynTask(String sql, final Context context, RelativeLayout relativeLayout) {
		this.sql = sql;
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				((Activity)context).runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						if(null != context && GetMyQuestionAsynTask.this.isCancelled())
	...
	@Override
		...
		try {
			...
			GetMyQuestionAsynTask.this.cancel(true);
	...
	protected void onPostExecute(Void v) {
		...
		try{
			if(result.equals("false")){
				...
				GlobalData.myQuestions = questions;
